<?php	
		...
		isset($_POST["gender"]))
	{
			...
			($_POST["gender"] == "Homme" OR $_POST["gender"] == "Femme"))
		{
			...
			if(isset($_POST["lastname"])){$lastname = htmlspecialchars($_POST["lastname"]);}else{$lastname = 'Inconnu';}
			if(isset($_POST["firstname"])){$firstname = htmlspecialchars($_POST["firstname"]);}else{$firstname = 'Inconnu';}
			if(isset($_POST["address"])){$address = htmlspecialchars($_POST["address"]);}else{$address = 'Inconnu';}
			if(isset($_POST["address2"])){$address2 = htmlspecialchars($_POST["address2"]);}else{$address2 = '';}
			if(isset($_POST["city"])){$city = htmlspecialchars($_POST["city"]);}else{$city = 'Inconnu';}
			if(isset($_POST["zipcode"])){$zipcode = htmlspecialchars($_POST["zipcode"]);}else{$zipcode = 'Inconnu';}
			if(isset($_POST["country"])){$country = htmlspecialchars($_POST["country"]);}else{$country = 'Inconnu';}
			
			$password = sha1($password);
			
			$signup = insert_new_client($username,$password,$email,$year,$month,$day,$gender,$firstname,$lastname,$address,$address2,$city,$country,$zipcode);
